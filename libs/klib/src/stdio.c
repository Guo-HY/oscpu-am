...
int printf(const char *fmt, ...) {
  ...
  va_list ap;
  va_start(ap, fmt);
  l = user_lp_Print(user_myoutput, 0, fmt, ap);
  va_end(ap);
  return l;
}
...
int sprintf(char *out, const char *fmt, ...) {
  ...
  va_list ap;
  va_start(ap, fmt);
  l = user_lp_Print(user_out2string, out, fmt, ap);
  va_end(ap);
  return l;
}
...
/* forward declaration */
static int user_PrintChar(char *, char, int, int);
static int user_PrintString(char *, char *, int, int);
static int user_PrintNum(char *, unsigned long, int, int, int, int, char, int);
